    ...
    BEGIN
        ...
        for registro in detalle_operaciones loop -- Se recorre el cursor de detalle_operaciones
            DECLARE
                ...
                v_divisor_iva float;
            BEGIN
                v_precio_venta := c_precio_venta.precio_venta;

                -- Porcentaje descuento
                SELECT porcentaje_descuento INTO v_porcentaje_descuento 
                    FROM d_forma_pago WHERE cod_forma_pago = v_id_forma_pago;
                v_importe_descuento := (v_precio_venta*registro.cantidad_producto) * (v_porcentaje_descuento)/100;

                -- Porcentaje recargo                
                SELECT porcentaje_recargo INTO v_porcentaje_recargo
                    FROM d_forma_pago WHERE cod_forma_pago = v_id_forma_pago;
                v_importe_recargo := (v_precio_venta*registro.cantidad_producto) * (v_porcentaje_recargo)/100;
                
                -- Importe operación
                v_importe_operacion := v_precio_venta * registro.cantidad_producto;
                ...
                SET precio_venta = c_precio_venta.precio_venta,
